...
/*
 * Hashtable implementation of nested symbol tables
 */
...
/*
 * The lang supports nested scopes with redeclarations of local variables.
 ...
 * This is most simply done by assigning the entry number to to each variable 
 * which is guaranteed to be unique. Behold the symbol counter.
 */
...
void SymTab_dump(struct SymTab* symbol_table,  int indent)
{
    ...
    if (symbol_table->name != NULL)
        print_w_indent(indent, "%s", symbol_table->name);
    else
        print_w_indent(indent, "Local env", symbol_table->name);
    printf(" of %d symbols: \n", symbol_table->saturation);
